...
<script>
	var deletebookfailed = '<%=request.getParameter("deletebookfail")%>';
	if(deletebookfailed == 'no'){
		alert("图书下架成功！");
	}
	var deletereaderfailed = '<%=request.getParameter("deletereaderfail")%>';
	if(deletereaderfailed == 'no'){
		alert("用户删除成功！");
	}
	var updatebook = '<%=request.getParameter("updatebook")%>';
